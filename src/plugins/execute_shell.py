...
TOOL_SCHEMA = {
    ...
    "function": {
        "name": "execute_shell",
        "description": "Execute a short, read-only shell command in a locked‑down sandbox. Use sparingly for diagnostics (e.g., 'git status', 'ls'). Disabled by default; requires SHELL_TOOL_ALLOW=1 and allowlisted prefixes. Execution is path‑confined to SHELL_TOOL_ROOT, may be non‑networked by policy, and only a compact summary is injected for context.",
        "parameters": {
            "type": "object",
            "additionalProperties": False,
            "properties": {
                "command": {"type": "string", "description": "Exact command string. Must start with an allowlisted prefix. Use read‑only operations and do not include secrets."},
...
def execute_shell(command: str, working_dir: str = '.', timeout: int = 30, shell: bool = False, capture_stderr: bool = True, env_vars: Dict[str, str] | None = None) -> str:
    ...
    # Policy
    if not allow or not _allowed_command(cmd_str):
    ...
    res = run_in_sandbox(
        ...
        network=os.getenv('SANDBOX_NET', 'deny'),
